...
function phonePeChecksum(payload) {
  ...
  const hash   = crypto.createHash('sha256').update(base64 + '/pg/v1/pay' + SALT_KEY).digest('hex');
  return { base64, checksum: `${hash}###${SALT_INDEX}` };
}

function verifyCallback(body, xVerify) {
  const [receivedHash] = xVerify.split('###');
  const expected = crypto.createHash('sha256').update(body + SALT_KEY).digest('hex');
...
// POST /api/orders/phonepe-callback
exports.phonePeCallback = async (req, res) => {
  ...
  try {
    await pool.query(
      ...
      [success ? 'paid' : 'failed', transactionId, success ? 'confirmed' : 'pending', merchantTransactionId]
    );
...
exports.place = async (req, res) => {
  const {
    shipping_address,
    payment_method,        // 'cod' | 'whatsapp' | 'phonepe' | 'razorpay'
  ...
  const validMethods = ['cod', 'whatsapp', 'phonepe', 'razorpay'];
  ...
  try {
    ...
    // cod       → confirmed  / unpaid   (will pay on delivery)
    // whatsapp  → pending    / unpaid   (admin confirms after WhatsApp chat)
    // phonepe   → pending    / unpaid   (payment pending)
    // razorpay  → confirmed  / paid     (payment already verified)
    const statusMap = {
      cod:      { status: 'confirmed', payment_status: 'unpaid'  },
      whatsapp: { status: 'pending',   payment_status: 'unpaid'  },
      phonepe:  { status: 'pending',   payment_status: 'unpaid'  },
      razorpay: { status: 'confirmed', payment_status: 'paid'    },
    };
    const { status: orderStatus, payment_status } = statusMap[payment_method];
    ...
    for (const item of cartItems) {
      ...
      await client.query(
        ...
        [order.id, item.product_id, item.name, img, item.product_type,
...
exports.adminList = async (req, res) => {
  const { page = 1, limit = 20, status, search, payment_method } = req.query;
  ...
  if (payment_method) { conds.push(`o.payment_method=$${n++}`);  params.push(payment_method); }
  if (search) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT o.*,
              ...
              STRING_AGG(oi.product_name, ', ' ORDER BY oi.id) as item_names
       FROM orders o
       LEFT JOIN users u ON o.user_id=u.id
       LEFT JOIN order_items oi ON o.id=oi.order_id
       ${where}
       GROUP BY o.id, u.name, u.email
       ORDER BY o.created_at DESC LIMIT $${n} OFFSET $${n+1}`,
...
exports.updateStatus = async (req, res) => {
  ...
  try {
    ...
    if (status)         { query += `, status=$${n++}`;         vals.push(status); }
    if (payment_status) { query += `, payment_status=$${n++}`; vals.push(payment_status); }

    query += ` WHERE id=$${n} RETURNING *`;
...
exports.dashboard = async (req, res) => {
  try {
    const [rev, orderStats, paymentStats, customers, products, recent, topProducts, monthlySales] = await Promise.all([
      ...
      // WhatsApp orders count
      pool.query(`SELECT payment_method, COUNT(*) as count FROM orders GROUP BY payment_method ORDER BY count DESC`),
